...
model2.load_state_dict(torch.load(os.path.join(check_path2, "best_metric_model.pth"), map_location=torch.device('cpu')), strict=False)
model2.to(device)
...
#      .cuda()
      ...
      fl_run_inputs = torch.unsqueeze(img, 1)
