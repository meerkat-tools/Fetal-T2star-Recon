...
#nifti_directory = case_dir + '/ME/n' + nr_me + '/processing_body/out-proc/'
print('NIFTI directory: ' + nifti_directory)

# get echo filenames
echo_filenames = []
for echo in range(int(num_echos)):
    echo_location = nifti_directory + 'recon_struct_body_e0' + str(echo) + '.nii.gz'
    # echo_location = 'recon_struct_body_e0' + str(echo) + '.nii.gz'
...
t2map_path =  nifti_directory + 't2map_from_recon_body.nii.gz'
...
# read in te's from text file
te_file = glob.glob(case_dir + '/ME/n' + nr_me + '/*txt')
read_te_file = open(te_file[0] ,'r')

te_content=read_te_file.read().splitlines() 
te = sorted([float(i) for i in te_content])
...
for iz in range(0,nO_slices):
    ...
    for ix in range(0,x_dim):
        for iy in range(0,y_dim):
            # 
            if nifti_concat[ix,iy,iz,int(num_echos)-1] != 0:
                # print(nifti_concat[ix,iy,iz,:])
                # print(ix,iy,iy)
                pix_array = np.array(nifti_concat[ix,iy,iz, :], dtype=float)
                param_init = np.squeeze([pix_array[2], np.average(te)])
                # print(pix_array)
                try:
                    result = least_squares(t2fit, param_init, args = (pix_array,te), bounds=([0,0],[100000,1000]))
                    T2_map[ix,iy,iz, 0]= result.x[0]
                    T2_map[ix,iy,iz, 1]= result.x[1]
                except ValueError:
                    T2_map[ix,iy,iz, 0]= 1000
                    T2_map[ix,iy,iz, 1]= 1000
