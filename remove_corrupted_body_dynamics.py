...
python remove_corrupted_body_dynamics.py [file location] [sequence num] [number of echos] [dyn to exclude]
...
for x in range(1,int(nr_echos)+1):
    if len(glob.glob(nifti_directory+'*e0*' + str(x) + '*')) != 0:

        echo_filenames.append(glob.glob(nifti_directory+'*e0' + str(x) + '*')[0])
    else:
...
if not os.path.exists(nifti_directory + 'files_for_body_recon/'):
    os.mkdir(nifti_directory + 'files_for_body_recon/')
...
for x,file in enumerate(echo_filenames):
    ...
    nib.save(new_image, nifti_directory + 'files_for_body_recon/e' +  str(x+1) + '.nii.gz')
...
if not os.path.exists(nifti_directory + 'processing_body'):
    os.mkdir(nifti_directory + 'processing_body')

# perform concatenation and denoising
if not os.path.isfile(nifti_directory + 'concat_denoised.nii.gz'):
    new_echos = os.listdir(nifti_directory + 'files_for_body_recon/')

    for x,file in enumerate(new_echos):
        echo_img = nib.load(nifti_directory + 'files_for_body_recon/' + file)
        nifti_echo_img = echo_img.get_fdata()
        if x ==0:
...
for x in range(0,int(nr_echos)):
    ...
    nib.save(new_denoised_image, nifti_directory + 'processing_body/e' +  str(x+1) + '_denoised.nii.gz')
