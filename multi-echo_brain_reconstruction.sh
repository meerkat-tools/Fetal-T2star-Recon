...
cd out-proc
echo mirtk reconstruct ${roi_recon}-output.nii.gz ${number_of_stacks} ../stack-t2s-e02/recon-stacks-brain/*.nii.gz --mc_n $nr_channels --mc_stacks ${channel_text} -mask ../stack-t2s-e02/average_mask_cnn.nii.gz -default_thickness ${default_thickness} -iterations 2 -no_robust_statistics -resolution ${default_resolution} -delta 150 -lambda 0.02 -structural -lastIter 0.015 -no_intensity_matching
...
mv SVR-output.nii.gz recon_struct_brain_e0${nr_channel}.nii.gz
fi
done

python ${SCRIPT_DIR}/t2s_fitting_brain.py ${org_files} ${nr_me} ${nr_echos}
...
mirtk prepare-for-monai res-met2srecon stack-met2srecon met2srecon-info.json met2srecon-info.csv ${res} 1 recon_struct_brain_e02.nii.gz
...
for nr_channel in $(seq 0 $nr_channels); do
mirtk edit-image recon_struct_brain_e0${nr_channel}.nii.gz ../../reconstructions/recon_struct_brain_e0${nr_channel}.nii.gz -dofin_i reo-dofs/dof-to-atl.dof
mirtk transform-image ../../reconstructions/recon_struct_brain_e0${nr_channel}.nii.gz ../../reconstructions/recon_struct_brain_e0${nr_channel}.nii.gz -target ${brain_recon}

done

mirtk edit-image t2map_from_recon_brain.nii.gz ../../reconstructions/t2map_from_recon_brain.nii.gz -dofin_i reo-dofs/dof-to-atl.dof
mirtk transform-image ../../reconstructions/t2map_from_recon_brain.nii.gz ../../reconstructions/t2map_from_recon_brain.nii.gz -target ${brain_recon}

cd ../../
for nr_channel in $(seq 0 $nr_channels); do
mirtk edit-image reconstructions/recon_struct_brain_e0${nr_channel}.nii.gz reconstructions/recon_struct_brain_e0${nr_channel}.nii.gz -origin 0 0 0 

done

mirtk edit-image reconstructions/t2map_from_recon_brain.nii.gz reconstructions/t2map_from_recon_brain.nii.gz -origin 0 0 0    
