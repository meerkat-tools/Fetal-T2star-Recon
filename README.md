...
1. Format the input data. The script requires a certain folder setup. All files for each subject should be in a folder called ‘ME’ (multi-echo), and within the ME folder there should be a folder for each sequence you want to process. These are called ‘n[sequence number]’ and they should contain the niftis. The nifti files should be 4D, each file should contain all the dynamics for each echo. The text ‘e1’, ‘e2’, ‘e3’, etc should be in the filename. 
   a.	$ mkdir /home/user/case0001/ME
   b.	$ mkdir /home/user/case0001/ME/n[scan_number] 
   	i.e. $ mkdir /home/user/case0001/ME/n17
   
2. Place a text file in the nifti folder called 'tes.txt' that lists the echo times (can be in ms or s). 

3. Review all acquired dynamics, note which dynamics (if any) you would like to exclude from the reconstruction due to motion artefact, noise, etc. (dynamic numbering starts at 1 (NOT 0). If no dynamics are being excluded, put 0.
   
4.	 Run FOREST:
   a.	 $ source forest_body.sh [complete path to folder with files] [scan_number] [number of echos] [dyn to exclude]
         i.e. forest_body.sh /home/user/case0001 59 3 0 – no dynamics excluded
         i.e. forest_body.sh /home/user/case0001 59 3 2,3,4,5,12 – 2nd,3rd,4th, 5th, and 12th dynamic excluded from reconstruction
  	 
5.	If the automated body masking fails, there is the option to use a manual mask. This can be added as an additional input argument:
    a.	$ source forest_body.sh  /home/user/case0001 59 3 0 /home/user/case0001/case0001_body_mask.nii.gz
